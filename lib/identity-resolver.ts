...
/** An owner counts as active unless explicitly active===false, OR their Greenhouse user record
 *  says `deactivated`. Both signals are needed because they cover different paths: the YTD
 *  snapshot path maintains its own `active` flag (ytd-extract's OWNER_STALE_GRACE_MS), while the
 *  live /job_owners projection omits `active` entirely (greenhouse-evidence.ts:385) — so on the
 *  sweep paths `deactivated` off /v3/users is the ONLY evidence a departed owner leaves behind.
 *  Without it a departed recruiter could win the election and be written to
 *  ytd_application_facts.primary_recruiter_id, which is how 38 applications came to be attributed
 *  to people who had left (audited 2026-08-14).
 *
 *  An id absent from usersById is treated as ACTIVE — a missing record is not evidence of
 *  departure, and callers legitimately pass partial maps (ytd-normalize seeds name-only stubs for
 *  owners whose user record was not separately fetched). */
function isActiveOwner(o: OwnerRow, usersById: Map<number, YtdGHUser>): boolean {
  if (o.active === false) return false
  return usersById.get(o.user_id)?.deactivated !== true
}
...
export function resolveOwnership(ev: OwnershipEvidence): OwnershipResolution {
  ...
  const recruiterOwners = (ev.jobOwners ?? []).filter(
    (o) => o.type === RECRUITER_TYPE && isActiveOwner(o, usersById)
  )
