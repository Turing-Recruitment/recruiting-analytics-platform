...
export async function runReferralSweep(opts: {
  ...
  try {
    // Step 1: Fetch the alertable universe in two slices (CLO-172 defect 1).
    //
    ...
    //     The census is SCOPED SERVER-SIDE to the open job ids (job_ids filter, batched <=50).
    //     It ran unscoped until 2026-08-18 and asked Greenhouse to walk the tenant's whole
    //     application id range; Greenhouse answers that with a ~28s gateway timeout and a 503.
    //     Measured that day: unscoped failed 4 of 4 trials, scoped succeeded 4 of 4 in 1.5-3.7s.
    //     The unscoped form drove this sweep from a 0% to a 96% daily failure rate in six days,
    //     including a 30-hour unbroken silence (2026-08-16 16:00Z - 2026-08-17 21:00Z). Scoping is
    //     also strictly less work: 97% of the unscoped result sat on closed reqs and was thrown
    //     away client-side. The open-job fetch therefore has to resolve BEFORE the census — it is
    //     the census's scope, not a post-filter.
    const openJobs = await greenhouseGetAll<GHJob>("/jobs", {
      status: "open",
      per_page: 500,
    })
    const openJobIds = new Set(openJobs.map((j) => j.id))

    const [recentApps, censusAppsAllJobs] = await Promise.all([
      ...
      openJobIds.size === 0
        ? Promise.resolve([] as GHApplication[])
        : listApplicationsForJobs([...openJobIds], {
            source_ids: SWEEP_CONFIG.referral.sourceIds.join(","),
            status: "active",
    ...
    const items: ReferralSweepItemWithIdentity[] = applications.map((app) => {
      ...
      // collapsed to null.
      //
      // DEPARTED OWNERS are excluded (Sam's rule, 2026-08-14: "if someone has departed the
      // company, no message should be sent out"). A deactivated user still seated as a Recruiter
      // was producing DMs addressed to someone who had left; unreachable recipients fall back to
      // the head-of-TA, so those copies landed on him. /v3/users carries `deactivated` and this
      // sweep already fetches it above for name resolution — it was simply never consulted.
      // A user id absent from usersById is treated as ACTIVE: a missing record is not evidence of
      // departure, and dropping an owner on missing evidence would silently un-notify a live
      // recruiter. A req whose recruiters have ALL departed yields an empty list, which routes to
      // the head-of-TA as an unowned requisition — the correct signal that it needs restaffing,
      // and a different message from a copy addressed to someone who left.
      const recruiterIds = [
        ...new Set(
          jobOwnerRows
            .filter(
              (o) =>
                o.type === "recruiter" &&
                usersById.get(o.user_id)?.deactivated !== true
            )
            .map((o) => o.user_id)
        ),
      ]
    ...
    // ALERTABLE new items only (CLO-172 defect 4): an item already actioned at first sight was
