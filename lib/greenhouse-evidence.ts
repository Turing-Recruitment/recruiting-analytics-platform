...
import type { GHApplication, GHCandidate, GHJob } from "./sweep-types"
...
/** Applications on the given jobs, batched <=50 job ids/query.
 *
 *  The referral census MUST be scoped this way. An unscoped `status + stage_name` scan makes
 *  Greenhouse walk the tenant's entire application id range, and it answers with a ~28s gateway
 *  timeout and a 503: measured 2026-08-18, the unscoped query failed 4 of 4 trials while the same
 *  query scoped to the 51 open job ids succeeded 4 of 4 in 1.5-3.7s. The unscoped form had driven
 *  the referral sweep from a 0% to a 96% failure rate over six days, including a 30-hour unbroken
 *  silence. Scoping is also strictly less work for us: 97% of the unscoped result sat on closed
 *  requisitions and was discarded client-side (1,681 of 1,728 on the 2026-08-12 census). */
export async function listApplicationsForJobs(
  jobIds: Array<number | null | undefined>,
  extra?: Record<string, string | number | boolean | undefined>
): Promise<GHApplication[]> {
  return fetchByIds<GHApplication>("/applications", "job_ids", jobIds, extra)
}
