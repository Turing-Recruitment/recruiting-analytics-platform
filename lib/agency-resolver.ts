 ...
 *        - isAgencySource(src)            : the CBREX-collision-safe type gate
 ...
 * list contains a Prospecting source named "CBREX" whose OWN id is 4000007004 — the same numeral
 * as the "Agencies" source TYPE id. Selecting agencies by `src.id === 4000007004` therefore seeds
 * exactly one bogus row (CBREX) and ZERO real agencies. Selection MUST key on `src.type.id`. That
...
// Prospecting source ("CBREX"). Everything in this module that decides "is this an agency"
// compares against source.type.id, never source.id.
// ---------------------------------------------------------------------------

export const AGENCY_SOURCE_TYPE_ID = 4000007004

/** The Prospecting type id that "CBREX" actually carries — kept for documentation/clarity of
...
/**
 * The CBREX-safe agency gate. TRUE only when the source's TYPE is the Agencies type.
 * Keying on src.type.id (correct) rejects CBREX, whose own id collides with the type id but
...
// the injected registry, SELECTS the agency-TYPE rows (the CBREX-safe gate — type.id only),
 ...
 * incl. the CBREX collision row) are DROPPED before any registry consultation, so an id-keyed
 * registry hit on CBREX can never resurrect it (selection is on type.id, the bug-proof axis).
 */
export function resolveAllAgencySources(
  ...
  for (const src of sources) {
    // Selection gate FIRST — on type.id only. CBREX (own id === AGENCY_SOURCE_TYPE_ID, type
 ...
 * AGENCY_SOURCE_TYPE_ID). The CBREX row is excluded because its type is Prospecting, even though
 ...
 * agency-TYPE subset (CBREX-safe — type.id only), resolves each against an empty registry
...
export async function bootstrapAgencySources(
  ...
  // type.id inside resolveAllAgencySources, so CBREX and job boards never reach the upsert.
