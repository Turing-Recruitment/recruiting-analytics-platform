...
const CORPORATE_EMAIL = /^[A-Z0-9.!#$%&'*+/=?^_`{|}~-]+@turing\.com$/i
...
export function readEmployeeReferralRecipientConfig(
  ...
  if (
    Object.keys(record).sort().join(",") !== "kelsey,requesting_manager" ||
    typeof record.kelsey !== "string" ||
  ...
  const kelsey = normalizeCorporateAddress(record.kelsey)
  const requestingManager = normalizeCorporateAddress(record.requesting_manager)
  if (kelsey === requestingManager) {
    throw new EmployeeReferralRunnerError(
      "recipient_configuration_invalid",
...
function requestFingerprintsForReport(
  ...
  return Object.fromEntries(
    (["kelsey", "requesting_manager"] as const).map((slot) => [
      slot,
...
function committedManualPreparationClaims(
  ...
  for (const delivery of state.deliveries) {
    if (
      ...
      (delivery.recipient_slot !== "kelsey" &&
  ...
  if (!deliveries.has("kelsey") || !deliveries.has("requesting_manager")) {
    return null
  }

  return (["kelsey", "requesting_manager"] as const).map((recipientSlot) => ({
