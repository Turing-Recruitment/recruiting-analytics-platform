...
export type EmployeeReferralRecipientSlot = "kelsey" | "requesting_manager"
...
function parseClaim(value: unknown): EmployeeReferralDeliveryClaim {
  ...
  if (
    ...
    (row.recipient_slot !== "kelsey" && row.recipient_slot !== "requesting_manager") ||
...
function validatePrepareInput(input: EmployeeReferralPrepareInput): void {
  ...
  for (const slot of ["kelsey", "requesting_manager"] as const) {
