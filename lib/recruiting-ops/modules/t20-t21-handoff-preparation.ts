...
function nextGateForReadiness(status: HandoffReadinessStatus): string {
  ...
  if (status === "ready") return "Request Sam signoff or owner acceptance."
  ...
  return "Clarify readiness state with Sam."
}
  ...
  for (const row of rows) {
    ...
    if (row.acceptance_required && row.sam_signoff_status !== "accepted") {
      gaps.push({
        ...
        reason: `Handoff area ${row.area_id} requires Sam acceptance before closeout.`,
        blocksCutover: true,
      })
    }
    if (row.owner === null) {
