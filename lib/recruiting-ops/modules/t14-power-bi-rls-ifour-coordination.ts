...
export interface IFourCoordinationFact {
...
export interface RunPowerBiRlsIFourCoordinationModuleInput {
  rootDir: string
  startedAt: string
  generatedAt: string
  rlsAccessFacts: readonly PowerBiRlsAccessFact[]
  vendorFacts: readonly IFourCoordinationFact[]
  legacyRows?: readonly LegacyPowerBiRlsEvidenceRow[]
}

export const powerBiRlsIFourCoordinationModuleDefinition = {
  moduleId: "t14-power-bi-rls-ifour-coordination",
  workflowId: "T14",
  capabilityId: "external_artifact_monitoring",
  title: "T14 Power BI RLS / iFour Coordination",
  sourceIds: ["power_bi", "google_sheets", "ifour"],
  queryIds: [],
  legacyArtifactIds: ["legacy_power_bi_rls_ifour_packet"],
...
const legacyArtifact = legacyArtifactRegistry.find((artifact) => artifact.id === "legacy_power_bi_rls_ifour_packet")!
if (!legacyArtifact) throw new Error("Missing Power BI RLS/iFour legacy artifact")
...
export async function runPowerBiRlsIFourCoordinationModule(
  input: RunPowerBiRlsIFourCoordinationModuleInput
): Promise<RecruitingOpsModuleResult<PowerBiRlsCoordinationRow>> {
  const runId = buildRunId(powerBiRlsIFourCoordinationModuleDefinition.workflowId, input.startedAt)
  ...
  const sourceRefs: SourceEvidenceRef[] = [
    {
      ...
      label: "Power BI RLS access matrix and iFour coordination evidence.",
      artifactId: legacyArtifact.id,
    },
  ]
  const publicSummary = {
    workflowId: powerBiRlsIFourCoordinationModuleDefinition.workflowId,
    moduleId: powerBiRlsIFourCoordinationModuleDefinition.moduleId,
  ...
  const run = buildCommandCenterRun({
    workflowId: powerBiRlsIFourCoordinationModuleDefinition.workflowId,
    capabilityId: powerBiRlsIFourCoordinationModuleDefinition.capabilityId,
    moduleId: powerBiRlsIFourCoordinationModuleDefinition.moduleId,
    mode: "fixture",
  ...
  return finalizeModuleResult({
    definition: powerBiRlsIFourCoordinationModuleDefinition,
    normalizedRows,
...
function buildPowerBiRlsSourceGaps(
  ...
  if (vendorFacts.length === 0) {
    gaps.push({
      id: "gap_t14_ifour_coordination_missing",
      workflowId: "T14",
      sourceId: "ifour",
      field: "vendorFacts",
      reason: "iFour coordination requires vendor ownership/payment context before local rendering.",
      blocksCutover: true,
    })
  }
  for (const row of rows) {
    if (row.status === "unknown") {
      gaps.push({
        ...
        workflowId: "T14",
        sourceId: row.row_type === "rls_access" ? "power_bi" : "ifour",
        field: "status",
...
function buildPowerBiRlsDiscrepancies(
  ...
  const discrepancies = sourceGaps.map((gap) =>
    buildDiscrepancy({
      ...
      legacyValueSummary: "Legacy Power BI/iFour evidence may contain this matrix or coordination field.",
