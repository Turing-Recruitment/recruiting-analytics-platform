...
Deliverables should tell Sam whether an output can be consumed, reviewed, sent, or must be blocked.
...
|---|---|---|
| `draft_only` | Local artifact exists but needs Sam review before use. | Command Center/local file only. |
...
| `ready_with_warnings` | Output can be reviewed or shared only if warnings are acknowledged. | Sam-approved local/exported delivery only. |
...
| `retired_by_signoff` | Legacy item has Sam signoff, replacement or accepted deletion, and rollback decision. | Hidden from active delivery; visible in legacy coverage. |
...
| `retired_by_signoff` | Sam has signed off; the capability's deliverables move to evidence/legacy coverage only. |
