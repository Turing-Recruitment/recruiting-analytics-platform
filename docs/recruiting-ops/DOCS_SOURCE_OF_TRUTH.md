...
Reference docs can inform transition risk, handover context, and provenance. They are not product architecture.

The transition-era reference set — the six-week transition-control implementation spec, the
transition-control operating plan, the handover automation assessment, and the aspirational
`RECRUITING_OPERATIONS_BRAIN_NORTH_STAR.md` — is not carried in this cut. It lives in the live
product repo, `Turing-Recruitment/ta-ops-analytics`, under `docs/recruiting-ops/`. The registries
here carry the surviving provenance entries. The north-star document in particular should not be
implemented from: its body reads as a mandate but contradicts active constraints
(Greenhouse-canonical, no production/live writes, the `capabilities.ts` registry, no broad PII).

## Archived Docs

Archived docs preserve the sequence of events and prior build basis. Do not implement from them.

The `archive/` tree is likewise not carried here; it is in the live product repo. Its three
documents were each superseded by a spec that this cut does ship at the top level of this
directory: `RECRUITING_OPS_COMMAND_CENTER_SPEC.md`,
`RECRUITING_OPS_COMMAND_CENTER_IMPLEMENTATION_PLAN.md`, and
`RECRUITING_REPORTING_PLATFORM_TECHNICAL_SPEC.md`. Read those instead.
