...
Prior passes treated "executive audience" as "surface-level information" and cut detail in the name of restraint (a "when in doubt, hide" rule inherited from a different product). Sam has ruled that out. The failure of the current page is **undifferentiated** detail — 20 rows shouting equally — not the presence of detail.
...
## 7. Questions only Sam can answer
