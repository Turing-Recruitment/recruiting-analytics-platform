...
project `ilkbfyubwvbpsevybsfe`. It adds two functions and grants them to `service_role`; it
...
`ta-ops-hydrator-run@turing-recruiting.iam.gserviceaccount.com`, but `slack-bot-token` currently
...
gcloud secrets add-iam-policy-binding slack-bot-token \
  --project turing-recruiting \
  --member "serviceAccount:ta-ops-hydrator-run@turing-recruiting.iam.gserviceaccount.com" \
...
gcloud run jobs update ta-ops-staging-hydration \
  --region us-west1 --project turing-recruiting \
...
gcloud builds submit \
  --tag us-west1-docker.pkg.dev/turing-recruiting/recruiting/ta-ops-staging-hydrator:<sha> \
  --project turing-recruiting
```
...
gcloud run jobs update ta-ops-staging-hydration \
  --region us-west1 --project turing-recruiting \
  --image us-west1-docker.pkg.dev/turing-recruiting/recruiting/ta-ops-staging-hydrator:<sha> \
...
gcloud scheduler jobs create http recops-staging-hydration-watchdog \
  --location us-west1 --project turing-recruiting \
  --schedule "30 8 * * 1-5" \
  --time-zone "America/Los_Angeles" \
  --http-method GET \
  --uri "https://ta-ops-staging-hydrator-386190966264.us-west1.run.app/api/cron/recruiting-ops-staging-hydration-watchdog" \
  --oidc-service-account-email "ta-ops-hydration-scheduler@turing-recruiting.iam.gserviceaccount.com" \
  --oidc-token-audience "https://ta-ops-staging-hydrator-386190966264.us-west1.run.app" \
...
Sam's call, 2026-08-07: the schedule waits until every artifact writes cleanly rather than going
...
gcloud run jobs executions list --job ta-ops-staging-hydration \
  --region us-west1 --project turing-recruiting --limit 3
```
