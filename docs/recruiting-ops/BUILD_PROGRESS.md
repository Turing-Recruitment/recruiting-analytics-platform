...
Status: Active
Owner: Sam
Worktree: `/Users/sam.vangelos/Projects/recruiting-tools/ta-ops-analytics-automation-control-plane-phase1-6`
...
The audit remediation campaign (plan: phases A–D) is executing. **Phases A, B, C1/C1b, C2, and C3's code side (C3-1 container, C3-2 trigger) are COMPLETE.** Next: C3-3 GCP substrate (Artifact Registry image, Cloud Run service, Scheduler jobs created PAUSED, Secret Manager) — built dark next to prod; Sam gates the cutover repoint and the G2 shadow flip. Then C4 delivery (T18 Slack first).

## 2026-07-07: C3-3 — GCP substrate LIVE (Cloud Run + Secret Manager + paused Scheduler)

Sam took ownership of `turing-recruiting`; the dark substrate is now deployed and verified:

- **Image:** Cloud Build from the working tree (explicit `.gcloudignore` — env files and `.recruiting-ops-artifacts` can never enter the source upload) → `us-west1-docker.pkg.dev/turing-recruiting/recruiting/ta-ops-analytics:7470e31` (2m51s, the C3-1 Dockerfile unchanged).
- **Service:** `ta-ops-analytics` in us-west1 (Supabase sits in AWS us-west-2 — same metro), dedicated runtime SA `ta-ops-analytics-run@` with per-secret accessor grants only, 1Gi/1cpu, min 0 / max 2, 300s timeout, env via Secret Manager (7 secrets), `SUPABASE_URL` as plain env. URL: https://ta-ops-analytics-386190966264.us-west1.run.app
...
- **Scheduler:** six HTTP jobs created and immediately PAUSED (sweep-referral hourly, sweep-agency 4h, ytd-incremental 06:30, reconcile-identity 06:00, notify-drain :15, recruiting-ops-exec :45 — all UTC, Bearer CRON_SECRET header, 320s deadline). Resuming them is Sam's cutover flip.
...
- **Sidecar retirement:** `recruiting-ops-exec-sheet-data.ts` + `recruiting-ops-exec-preview.ts` DELETED; `build-exec-workbook.py` marked DORMANT (future C4 export candidate only). New dormant trigger `/api/cron/recruiting-ops-exec` (CRON_SECRET fail-closed; `RECOPS_EXEC_ENABLED` exactly "true" — the flip is Sam's; set nowhere).
...
2. T05's residual module gaps are interview-stage TAXONOMY openness (Turing's slot names need governed vocabulary, same class as T02's stage dimension) — a governance table question for C2, not a mapping defect.
...
- Migrations renumbered 010/011/012 → **015/016/017** (git mv; origin/main's parallel modernization lineage occupies 010–014; these were never applied anywhere). Headers document the renumber; the 017 do-not-apply-until-Sam-approves gate is preserved.
...
- Live Greenhouse credentials/reads/writes, external delivery, production writes, production persistence migration/backfill, broad PII persistence, scoped-MCP imports, UI mutation controls, and legacy retirement/cutover remain blocked until Sam explicitly approves a separate boundary.
...
- First real integration boundary planning should start with live Greenhouse read approval, credential handling, source freshness contracts, and production persistence review; do not proceed without Sam approval.
...
- Continue post-hardening review of any remaining low-priority items; keep all follow-up work local-only unless Sam approves a hard-stop boundary.
...
- External-channel delivery, production adapters, live Greenhouse writes, broad PII persistence, production persistence migrations, legacy retirement, and cutover remain blocked until Sam explicitly scopes a future gate.
...
- Await Sam direction for the next post-Phase-0 workstream.
...
- Updated architecture checks to require the new active docs and reject stale Sam-review-specific readiness terminology.
...
- Live persistence adapters remain disabled unless Sam explicitly scopes them into a future goal.
...
- T10 remains dormant by design; execution requires explicit Sam approval and current-consumer validation.
...
- Implement T14 Power BI RLS / iFour Coordination as a local access-matrix and vendor-coordination evidence module.

## 2026-06-24: T14 Power BI RLS / iFour Coordination Module

Completed gate:

- Implemented `t14-power-bi-rls-ifour-coordination` as a local access-matrix and vendor-coordination evidence module.
- Added a concrete local output contract for `power_bi_rls_matrix`.
- Registered `legacy_power_bi_rls_ifour_packet` as Power BI/iFour evidence and compatibility target.
- Normalized RLS access facts and iFour coordination facts into review-gated local rows.
- Classified missing RLS matrix, missing vendor context, open status taxonomy, and unmapped owners as source gaps.
- Rendered local JSON and CSV artifacts only; no Power BI, iFour, payment, or Google Sheet adapter was added.

Files changed:

- `lib/recruiting-ops/index.ts`
- `lib/recruiting-ops/modules/t14-power-bi-rls-ifour-coordination.ts`
- `lib/recruiting-ops/output-contracts.ts`
- `lib/recruiting-ops/legacy-artifact-registry.ts`
- `test/recruiting-ops-t14-power-bi-rls-ifour-coordination.test.ts`
- `test/recruiting-ops-substrate.test.ts`

Commands run:

- `npm test -- test/recruiting-ops-t14-power-bi-rls-ifour-coordination.test.ts test/recruiting-ops-substrate.test.ts`
- `npm run check:recruiting-ops`
- `npm test`
- `git diff --check`
...
- Actual RLS matrix, iFour owner, payment status, and support context still need capture.
...
- Cutover/retirement remains blocked until Sam approves after witnessed evidence.
...
- Retiring or replacing scripts remains out of scope until Sam approves after witnessed evidence.
...
- Drafts remain human-reviewed and human-sent until Sam explicitly approves a later adapter gate.
...
- Implement T20/T21 Sam Handoff Preparation as a local readiness dashboard over workflow coverage, access custody, SOP closure, and acceptance gates.

## 2026-06-24: T20/T21 Sam Handoff Preparation Module

Completed gate:

- Implemented `t20-t21-handoff-preparation` as a local handoff-readiness dashboard over workflow coverage, access custody, SOP closure, evidence, and Sam acceptance gates.
...
- Normalized readiness facts and Sam signoff facts into accepted, ready, needs-evidence, blocked, not-started, and unknown states.
- Classified missing readiness facts, missing evidence, unresolved blockers, missing Sam signoff, and unmapped owners as source gaps.
...
- Final readiness checklist, required evidence semantics, and Sam acceptance semantics still need capture.
...
- Any live persistence adapter, backfill, hosted DB migration, or production data movement requires explicit Sam approval.
...
- Enabling live persistence reads, production writes, external network calls, or live action adapters requires explicit Sam approval.
...
- Await Sam approval for any post-foundation workstream: live read adapters, Supabase persistence adapter/backfill, production output adapters, or action execution paths.
...
Executes the approved `docs/recruiting-ops/exec-design/EXEC_SURFACE_CONTENT_SPEC.md` (Sam sign-off 2026-07-08: descriptive voice on non-search tiers, one-shot ship, recruiter-name adjacency kept).
...
Deferred: none blocking. Design-iteration pass on the rendered page with Sam; ELT/legacy lanes untouched.
