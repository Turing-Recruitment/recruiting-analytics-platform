...
> **2026-08-06 canonical cutover.** Per Sam's directive, the mutation registry
> now targets the 11 CANONICAL artifacts directly; the Sam-owned copies below
...
This runbook operated the eleven registered Sam-owned copies pre-cutover. Sam
...
GCLOUD=/Users/sam.vangelos/google-cloud-sdk/bin/gcloud
```
...
`fbcae9d8d679061dd210e58e8f38bec07957d205` on immutable digest
`sha256:f379fc76c6e0ec375aa83eb9e9ed0e19bed0f09da184bc09603a1633799b573c`.
...
is `ta-ops-staging-hydration-vx4hm`.
...
operations team on Sam's behalf.
...
reconciliation only; differences caused by Sam's manual canonical updates are
not copied back or treated as a request to mutate canonical content.

## Start recurring registered-copy updates later

No recurring Google-file updates are enabled by this closure. When Sam later
asks to start them:
...
while Sam is continuing to update the files manually.
...
After Sam authorizes recurring copy updates and one unified cycle succeeds,
