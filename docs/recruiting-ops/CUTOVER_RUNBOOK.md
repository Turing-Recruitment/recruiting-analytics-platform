...
- **Vercel production is NOT frozen at the June state.** A CLI deploy went out
  ...
  theoretical.
- **Cloud Run `ta-ops-analytics` (us-west1) is live and fully provisioned:**
...
- **Vercel REST API token on this machine is dead** (`invalidToken`); the
  Vercel **CLI is authenticated** (`vercel whoami` → sam-vangelos). All Vercel
...
```bash
cd /Users/sam.vangelos/Projects/recruiting-tools/ta-ops-analytics
...
for j in sweep-referral sweep-agency ytd-incremental reconcile-identity notify-drain; do
  gcloud scheduler jobs resume "$j" --project turing-recruiting --location us-west1
done
gcloud scheduler jobs list --project turing-recruiting --location us-west1 --format="value(ID,STATE)"
```
...
export PATH="$HOME/google-cloud-sdk/bin:$PATH"
gcloud scheduler jobs describe sweep-referral --project turing-recruiting --location us-west1 \
...
# stop GCP
for j in sweep-referral sweep-agency ytd-incremental reconcile-identity notify-drain; do
  gcloud scheduler jobs pause "$j" --project turing-recruiting --location us-west1
done
