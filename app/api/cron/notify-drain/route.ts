...
import { runSweepWatchdog } from "@/lib/sweep-watchdog"
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Sweep watchdog. It lives HERE, in the drain, and not inside a sweep on purpose: a check
    // that runs inside the process being watched cannot report that process failing to start or
    // failing every time. The drain is hourly and independent of the sweep lanes. It never
    // throws — its result is reported, so a watchdog fault degrades to silence rather than
    // taking down send. See lib/sweep-watchdog.ts for the stateless throttle.
    const watchdog = await runSweepWatchdog()
