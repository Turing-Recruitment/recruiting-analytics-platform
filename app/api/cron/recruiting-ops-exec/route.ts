 ...
 * Supabase write. The flag is set nowhere until Sam flips it (the
...
export async function GET(request: Request) {
  ...
  if (readEnv("RECOPS_EXEC_ENABLED")?.trim() !== "true") {
    return noStoreJson({
      ...
      status: "disabled",
      detail: "RECOPS_EXEC_ENABLED is not 'true'; the exec lane is dormant by design (the flip is Sam's).",
    })
  }
