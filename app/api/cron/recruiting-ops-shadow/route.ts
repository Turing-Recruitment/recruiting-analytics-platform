 ...
 * Supabase write. Flipping the env is gate G2 (Sam). The route exists on every
...
export async function GET(request: Request) {
  ...
  if (readEnv("RECOPS_SHADOW_ENABLED")?.trim() !== "true") {
    return noStoreJson({
      ...
      status: "disabled",
      detail: "RECOPS_SHADOW_ENABLED is not 'true'; the shadow lane is dormant by design (G2 is Sam's flip).",
    })
  }
