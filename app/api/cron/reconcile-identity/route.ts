 ...
 *                              + AgencyRegistryResolver (the CBREX-safe type.id gate).
...
async function runReconcile(opts: {
  ...
  if (dueSourceIds.length > 0) {
    ...
    for (const sourceId of dueSourceIds) {
      ...
      // itself the CBREX-safe gate — it fires only when sourceTypeId === AGENCY_SOURCE_TYPE_ID
      // — so passing the live source's own type.id straight through is correct: CBREX (own
