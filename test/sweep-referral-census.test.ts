// Census fetch locks for the referral sweep (CLO-172 defect 1 + defect 4).
//
...
// was structurally unreachable (zero breach rows across 12,002 sweep_items).
//
...
// sweep_items). Closed-job sitting referrals stay excluded — 1,681 of 1,728 sitting referrals on
// 2026-08-12 were on closed reqs, all non-actionable noise.
//
...
vi.mock("../lib/greenhouse-evidence", () => ({
  // The census is scoped server-side by open job ids (job_ids, batched <=50) — an unscoped
  // tenant-wide scan 503s. See test/sweep-referral-departed-and-scope.test.ts for the scope locks.
  listApplicationsForJobs: async (jobIds: Array<number | null | undefined>) => {
...
describe("referral sweep census fetch", () => {
  test("issues the recent-window query, the open-jobs query, and the scoped census", async () => {
    ...
    // the census is scoped to the open job ids, never a tenant-wide scan
    expect(gh.state.censusCalls).toEqual([[10]])
    expect(appCalls.filter((c) => !("created_at" in c.params))).toEqual([])
  })
