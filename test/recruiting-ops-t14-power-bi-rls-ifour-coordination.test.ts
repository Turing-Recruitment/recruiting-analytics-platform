...
import {
  ...
  powerBiRlsIFourCoordinationModuleDefinition,
  runPowerBiRlsIFourCoordinationModule,
  type IFourCoordinationFact,
  type PowerBiRlsAccessFact,
} from "../lib/recruiting-ops/modules/t14-power-bi-rls-ifour-coordination"
...
const vendorFacts: IFourCoordinationFact[] = [
  {
    coordinationId: "ifour_payment",
    vendorName: "iFour",
...
describe("T14 Power BI RLS / iFour coordination module", () => {
  test("declares the expected workflow, artifact, and output contracts", () => {
    expect(powerBiRlsIFourCoordinationModuleDefinition).toEqual({
  ...
  test("runs locally and writes RLS/vendor coordination artifacts", async () => {
    const result = await runPowerBiRlsIFourCoordinationModule({
      rootDir: tempRoot(),
  ...
  test("classifies legacy RLS/iFour coordination differences", async () => {
    const result = await runPowerBiRlsIFourCoordinationModule({
      rootDir: tempRoot(),
