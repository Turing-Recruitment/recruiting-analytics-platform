...
const PUBLIC_EXEC_ORIGIN =
  "https://ref-oidc-e49v5---ta-ops-analytics-7ocrxwr7lq-uw.a.run.app"
...
function rawScheduler(
  ...
  return {
    name: `projects/turing-recruiting/locations/us-west1/jobs/${expected.name}`,
    state,
...
function schedulerAssets(names: readonly string[]) {
  return names.map((name) => ({
    ...
    name: `//cloudscheduler.googleapis.com/projects/turing-recruiting/locations/us-west1/jobs/${name}`,
  }))
}
...
function service(name: string, image: string, sourceCommit: string) {
  ...
  return {
    metadata: {
      name: `projects/turing-recruiting/locations/us-west1/services/${name}`,
      generation: 1,
...
function job(
  ...
  return {
    metadata: {
      name: "projects/turing-recruiting/locations/us-west1/jobs/ta-ops-staging-hydration",
      generation: 1,
      labels: { "source-commit": sourceCommit },
    },
...
function withSchedulerCoverage<T extends Record<string, unknown>>(results: T) {
  return {
    ...results,
    schedulerLocations: [{ locationId: "us-west1" }],
    runtimeDiff: "",
  }
}
...
function liveResults(sha: string, image: string) {
  return withSchedulerCoverage({
    activeAccount: "sam.vangelos@turing.com",
    authProbe: true,
...
describe("secret-safe control-plane preflight", () => {
  ...
  test("governed Scheduler targets reject URI credentials and fragments", () => {
    ...
    for (const scheduler of raw) {
      ...
      uri.password = "secret"
      uri.hash = "candidate-sam-vangelos"
    ...
    expect(serialized).not.toContain("candidate-sam-vangelos")
  ...
  test("allowlisted control-plane fields cannot smuggle drift values into the snapshot", () => {
    const sha = "1".repeat(40)
    const image = `private@sha256:${"2".repeat(64)}`
    const marker = "candidate-sam-vangelos@example.com"
    const compactMarker = "candidatesamvangelos"
    ...
    const results = {
      ...
      privateService: {
        ...
        metadata: {
          ...
          labels: {
            ...
            "recops-scope": marker,
            "cloud.googleapis.com/location": "candidate-sam-vangelos",
          },
        },
        spec: {
      ...
      schedulerAssets: [
        {
          assetType: "cloudscheduler.googleapis.com/Job",
          name: `//cloudscheduler.googleapis.com/projects/turing-recruiting/locations/candidate-sam-vangelos/jobs/${EXPECTED_GOVERNED_SCHEDULERS[0].name}`,
        },
      ],
    }
    ...
    expect(JSON.stringify(snapshot)).not.toContain("candidate-sam-vangelos")
  ...
  test("fresh Scheduler or private-runtime failures block an otherwise-ready preflight", () => {
    ...
    const results = {
      ...
      jobLogs: [
        {
          timestamp: "candidate-sam-vangelos",
          severity: "ERROR",
        },
      ],
    }
  ...
  test("collection attempts every diagnostic after a failed posture read", () => {
    ...
    const collected = collectInputs(
      ...
      (spec) => {
        ...
        if (spec.id === "schedulerLocations") {
          return {
            status: 0,
            stdout: JSON.stringify([{ locationId: "us-west1" }]),
          }
        }
        if (spec.id === "schedulers:us-west1") {
    ...
    expect(collected.failures).toEqual([
      { id: "schedulers:us-west1", exitCode: 1 },
    ])
  ...
  test("invalid Scheduler location identifiers fail closed without entering diagnostics", () => {
    ...
    expect(attempted.at(-1)).toBe("schedulers:us-west1")
  })

  test("enabled public jobs block a fire and unrelated boundary crossings block independently", () => {
    const sha = "a".repeat(40)
    const image = `us-west1-docker.pkg.dev/project/repo/image@sha256:${"c".repeat(64)}`
