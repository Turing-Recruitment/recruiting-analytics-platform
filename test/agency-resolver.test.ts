...
// CBREX collision: AGENCY_SOURCE_TYPE_ID (4000007004) is BOTH the id of the "Agencies" source
// TYPE and the own id of an unrelated Prospecting source ("CBREX"). A resolver keying on
// source.id instead of source.type.id seeds CBREX and zero real agencies
...
// TYPE id, AND it is the own id of "CBREX" (a Prospecting source, type.id 4000003004).
...
function cbrexSource(overrides: Partial<GHSource> = {}): GHSource {
  return {
...
describe("isAgencySource — CBREX collision guard", () => {
  ...
  test("REJECTS CBREX: own id === 4000007004 but type is Prospecting", () => {
    const cbrex = cbrexSource()
    expect(cbrex.id).toBe(AGENCY_SOURCE_TYPE_ID) // the trap is real
    expect(cbrex.type?.id).toBe(PROSPECTING_TYPE_ID)
    // Keying on type.id (correct) rejects it; keying on src.id (the bug) would admit it.
    expect(isAgencySource(cbrex)).toBe(false)
  })
...
describe("resolveAllAgencySources — selection (type.id only) + the CBREX exclusion", () => {
  test("selects ONLY sources whose type.id === AGENCY_SOURCE_TYPE_ID and EXCLUDES CBREX", () => {
    const harnham = agencySource({ id: 5000001001, name: "Harnham" })
    const korn = agencySource({ id: 5000001002, name: "Korn Ferry" })
    const cbrex = cbrexSource()
    const linkedin = jobBoardSource()

    const sources: GHSource[] = [harnham, cbrex, korn, linkedin]
    const registry = stubRegistry([
      { source_id: harnham.id, source_name: "Harnham", agency_name: "Harnham" },
      { source_id: korn.id, source_name: "Korn Ferry", agency_name: "Korn Ferry" },
      // CBREX is deliberately ALSO present in the registry by its id, to prove that
      // selection happens on type.id BEFORE any registry consultation: an id-keyed
      // resolver would "successfully" resolve CBREX here, so its absence is load-bearing.
      { source_id: cbrex.id, source_name: "CBREX", agency_name: "CBREX (should never appear)" },
    ])

    const results = resolveAllAgencySources(sources, { registry })

    const ids = results.map((r) => r.source_id)
    expect(ids).toContain(harnham.id)
    expect(ids).toContain(korn.id)
    // The collision row is excluded purely on type.id, despite a registry hit on its id.
    expect(ids).not.toContain(cbrex.id)
  ...
  test("EXCLUDES CBREX even when it is the only candidate keyed by the colliding numeral", () => {
    // A list of nothing but the collision row + a non-agency must yield ZERO agencies.
    const results = resolveAllAgencySources([cbrexSource(), jobBoardSource()], {
      registry: stubRegistry([
        { source_id: AGENCY_SOURCE_TYPE_ID, source_name: "CBREX", agency_name: "CBREX" },
      ]),
    })
...
describe("resolveAllAgencySources — per-source resolution rungs", () => {
  test("a known source id resolves through the registry/resolution table (A1 -> confirmed)", () => {
    ...
    const [res] = resolveAllAgencySources([harnham], { registry })

    expect(res.source_id).toBe(harnham.id)
    expect(res.agency_name).toBe("Harnham")
  ...
  test("a source missing its name resolves by id through the registry", () => {
    ...
    const registry = stubRegistry([
      { source_id: 5000001003, source_name: "SingleSprout", agency_name: "SingleSprout" },
    ])

    const [res] = resolveAllAgencySources([nameless], { registry })

    expect(res.source_id).toBe(5000001003)
    expect(res.agency_name).toBe("SingleSprout")
    expect(res.status).toBe("resolved")
    expectContractInvariants(res)
  })
