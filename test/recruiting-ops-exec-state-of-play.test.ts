...
describe("eltFacts", () => {
  ...
  test("a declared backfill week rewindows the ELT block while generatedAt stays the live clock", () => {
    ...
    // derived Jun 26 - Jul 2 week); Alan Turing accepted daysAgo(30) = Jun 6.
    ...
    expect(bundle.eltFacts.hires[0]).toMatchObject({ candidate: "Alan Turing" })
  ...
  test.each([
    {
      ...
      extraStages: [{ id: 303, job_id: 1, name: "Turing Assessment" }],
    },
    {
  ...
  test("QTD counts section offers from the reporting quarter with names", () => {
    ...
    expect(codeSection.qtdOffers.names.sort()).toEqual(["Alan Turing", "Grace Hopper"])
