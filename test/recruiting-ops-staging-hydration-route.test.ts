...
const RECRUITING_OPS_STAGING_HYDRATION_JOB_RESOURCE =
  "projects/turing-recruiting/locations/us-west1/jobs/ta-ops-staging-hydration"
const RECRUITING_OPS_STAGING_ORCHESTRATION_SCHEDULER_JOB_NAME =
  "projects/turing-recruiting/locations/us-west1/jobs/recops-staging-orchestration-weekday"
...
describe("copy-only staging hydration route", () => {
  ...
  // The production regression of 2026-08-14..18: Cloud Scheduler delivers its
  // jitter INSIDE the schedule time (whole seconds — the job's stored fire
  // time was 06:30:03.469627Z), and this route refused every such fire as
  // off-second, so after the first jitter-free fire no scheduled run ever
  // launched. A jittered slot must launch, normalized to the slot minute.
  test("launches on a Scheduler fire whose schedule time carries whole-second jitter", async () => {
    vi.mocked(Date.now).mockReturnValue(Date.parse("2026-08-19T06:30:04.212Z"))
    const response = await POST(schedulerRequest({
      scheduledAt: "2026-08-19T06:30:03.469627Z",
    }))
    ...
    expect(GoogleAuth.prototype.request).toHaveBeenCalledWith(
      expect.objectContaining({
        data: {
          overrides: {
            containerOverrides: [{
              env: [{ name: "RECOPS_SCHEDULED_AT", value: "2026-08-19T06:30:00.000Z" }],
            }],
          },
        },
      })
  ...
  test("fails closed when the fixed Job or Scheduler configuration drifts", async () => {
    process.env[STAGING_HYDRATION_JOB_RESOURCE_ENV] =
      "projects/turing-recruiting/locations/us-west1/jobs/other-job"
    const errorLog = vi.spyOn(console, "error").mockImplementation(() => {})
    const response = await POST(schedulerRequest())
