...
const REPORT_JOB =
  "projects/turing-recruiting/locations/us-west1/jobs/employee-referral-monthly-report"
const REPORT_SUBJECT = "115001180678439780883"
...
describe("employee referral report cron route", () => {
  ...
  test("syncs once when authorized and returns aggregate state without PII", async () => {
    ...
    expect(serialized).not.toMatch(/candidate|referrer|@turing\.com/i)
  })

  test("surfaces failure as 500 while suppressing private diagnostics", async () => {
    const canary = "Private Candidate candidate@turing.com SECRET_REPORT_BODY"
