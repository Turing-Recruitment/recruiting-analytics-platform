...
describe("email transport", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("NOTIFY_EMAIL_FROM", "TA Ops <ta-ops@turing.com>")
  })
  ...
  test("preserves the existing call shape and requires a provider message ID", async () => {
    ...
    await expect(sendEmail("recipient@turing.com", "Subject", "<p>Body</p>")).resolves.toBe(
      "email_123"
    )
    ...
    expect(JSON.parse(String(init.body))).toEqual({
      from: "TA Ops <ta-ops@turing.com>",
      to: "recipient@turing.com",
      subject: "Subject",
      html: "<p>Body</p>",
    })
  })
  ...
  test("adds one CSV attachment and a stable provider idempotency key", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock)
    await sendEmail("recipient@turing.com", "Subject", "<p>Body</p>", {
      idempotencyKey: "employee-referral:period:1:kelsey",
      attachment: {
    ...
    expect(init.headers).toMatchObject({
      "Idempotency-Key": "employee-referral:period:1:kelsey",
    })
  ...
  test("turns provider rejection into a typed safe error without leaking its body", async () => {
    const canary = "recipient@turing.com SECRET_REPORT_BODY"
    vi.stubGlobal(
      "fetch",
    ...
    const error = await sendEmail("recipient@turing.com", "Subject", "<p>Body</p>").catch(
      (caught) => caught
    )
  ...
  test("classifies network loss and a 2xx response without an ID as ambiguous", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("recipient@turing.com CANARY")))
    const network = await sendEmail("recipient@turing.com", "Subject", "body").catch(
      (caught) => caught
    )
    ...
    const noId = await sendEmail("recipient@turing.com", "Subject", "body").catch(
      (caught) => caught
    )
  ...
  test("keeps the send deadline active while reading a stalled provider body", async () => {
    ...
    await expect(
      sendEmail("recipient@turing.com", "Subject", "body", { timeoutMs: 10 })
  ...
  test("rejects unsafe configuration before dispatch", async () => {
    ...
    await expect(sendEmail("recipient@turing.com", "Subject", "body")).rejects.toMatchObject({
      code: "resend_api_key_missing",
      kind: "configuration",
    })
    vi.stubEnv("RESEND_API_KEY", "test-key")
    await expect(
      sendEmail("recipient@turing.com", "Subject", "body", {
